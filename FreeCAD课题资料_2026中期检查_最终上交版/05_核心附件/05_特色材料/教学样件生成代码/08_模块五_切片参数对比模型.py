...
doc = App.newDocument("Module5_SliceModel")  # 新建教学模型文档。
...
model.Label = "切片参数对比模型（单实体）"  # 设置中文显示名称。
...
default_script_path = "/Users/wangzirui/教科研补件汇总/FreeCAD课题资料_按开题论证书设计/02_开发设计阶段_2025.07-12/03_教学样件_FCStd与STL/生成代码/08_模块五_切片参数对比模型.py"  # 提供 GUI 桥无 __file__ 时的兼容路径。
...
fcstd_path = os.path.normpath(os.path.join(script_dir, "..", "08_模块五_切片参数对比模型.FCStd"))  # 生成 FCStd 输出路径。
stl_path = os.path.normpath(os.path.join(script_dir, "..", "08_模块五_切片参数对比模型.stl"))  # 生成 STL 输出路径。
doc.saveAs(fcstd_path)  # 保存可编辑的 FreeCAD 文件。
Mesh.export([model], stl_path)  # 仅导出最终单实体，避免重叠网格。

print("切片参数对比模型生成完成")  # 输出完成提示。
