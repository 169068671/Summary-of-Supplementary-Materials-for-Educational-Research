...
fcstd_path = os.path.join(out_dir, "..", "06_模块一_3D打印启蒙模型.FCStd")  # FCStd 路径
doc.saveAs(fcstd_path)  # 保存
stl_path = os.path.join(out_dir, "..", "06_模块一_3D打印启蒙模型.stl")  # STL 路径
Mesh.export([box_obj, cyl_obj, sphere_obj, cone_obj, union_obj], stl_path)  # 导出全部5个几何体

# ---- 输出结果 ----
print(f"模块一启蒙模型生成完成：5个基础几何体（立方体+圆柱+球体+圆锥+组合）")
