...
DEFAULT_REMOTE = "https://github.com/169068671/Summary-of-Supplementary-Materials-for-Educational-Research.git"
DEFAULT_BRANCH = "main"
...
def is_git_repo(vault: Path) -> bool:
    result = git(vault, ["rev-parse", "--show-toplevel"], check=False)
    if result.returncode != 0 or not result.stdout.strip():
        return False
    return Path(result.stdout.strip()).resolve() == vault.resolve()
